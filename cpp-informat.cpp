...
// структура для хранения результата функции "my_sin"
...
// Функция для синуса через ряд Тейлора
// Параметры х - аргумент, е - точность
function_result_t my_sin(double x, double e)
{
    ...
    for (int k = 1; true; k++) 
    {
        ...
        if (fabs(iter) < e) 
        {
            return function_result_t(k-1, result);
        }
